...
export const useStockData = create<StockDataState>((set, get) => ({
  ...
  setDateRange: (range) => {
    ...
    const validRange = {
      from: range.from,
      to: range.to < range.from ? range.from : range.to,
    };

    set({
      dateRange: validRange,
      filteredData: filterDataByDateRange(get().data, validRange),
    });
  },
  ...
  searchStock: (query) => {
    ...
    const filtered = get().data.filter((item) => {
      ...
      const fromDate = startOfDay(get().dateRange.from);
      const toDate = endOfDay(get().dateRange.to);
...
export const calculatePercentageChange = (
  ...
  const lastValue = Number.parseFloat(data[0][field]);
  const firstValue = Number.parseFloat(data[data.length - 1][field]);
  return firstValue && !isNaN(firstValue) && !isNaN(lastValue)

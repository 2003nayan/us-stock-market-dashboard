...
export function StockHeatmap() {
  ...
  const [topStocks, setTopStocks] = useState<{ name: string; volume: number }[]>([])

  useEffect(() => {
    if (filteredData.length) {
      // Calculate total volume for each stock within the date range
      const volumeFields = Object.keys(filteredData[0]).filter((key) => key.includes("Vol"))
      ...
      const stockVolumes = volumeFields.map((field) => {
        ...
        const totalVolume = filteredData.reduce((sum, item) => {
          const volume = typeof item[field] === "number" ? item[field] : Number.parseFloat(item[field])
      ...
      const top16 = stockVolumes.sort((a, b) => b.volume - a.volume).slice(0, 16)
  ...
  const getColorIntensity = (volume: number) => {
    ...
    return `hsl(${Math.floor(220 - normalizedVolume * 220)}, 70%, ${Math.max(40, 70 - normalizedVolume * 30)}%)`
  }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Trading Volume Heatmap</CardTitle>
        <CardDescription>Top 16 most-traded stocks by total volume in selected period</CardDescription>
      </CardHeader>
      <CardContent>
          ...
          {topStocks.map((stock, index) => (
            <div
              ...
              style={{
                ...
                color: stock.volume > topStocks[0].volume / 2 ? "white" : "black",
              }}
            >

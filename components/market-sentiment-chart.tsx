...
export function MarketSentimentChart({
  ...
  const [sentimentConfig, setSentimentConfig] = useState<Record<string, { label: string; color: string }>>({})
  ...
  useEffect(() => {
    if (filteredData.length) {
      ...
      else stocksToAnalyze = [...techStocks, ...cryptoStocks, ...indicesStocks, ...commoditiesStocks, ...financeStocks]
      ...
      stocksToAnalyze.forEach((field) => {
        const firstValue = Number.parseFloat(filteredData[0][field])
        const lastValue = Number.parseFloat(filteredData[filteredData.length - 1][field])
            ...
            <PieChart>
              ...
              >
                {sentimentData.map((entry, index) => (
                  <Cell key={`cell-${index}`} fill={sentimentConfig[entry.name].color} />
                ))}
              </Pie>
              <ChartTooltip content={<ChartTooltipContent valueFormatter={(value) => `${value} stocks`} />} />
            </PieChart>
          </ResponsiveContainer>
        </ChartContainer>
      </CardContent>

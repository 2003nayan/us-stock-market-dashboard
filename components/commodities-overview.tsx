...
export function CommoditiesOverview() {
  ...
  const [commoditiesData, setCommoditiesData] = useState<{ name: string; price: number; change: number }[]>([])
  ...
  useEffect(() => {
    if (filteredData.length) {
      ...
      const data = commodities.map((commodity) => {
        const firstValue = Number.parseFloat(filteredData[0][commodity.field])
        const lastValue = Number.parseFloat(filteredData[filteredData.length - 1][commodity.field])
        const change =
      ...
      {commoditiesData.map((commodity, index) => (
        <Card key={index}>
          ...
          <CardContent>
            ...
            <p className={`text-xs ${commodity.change >= 0 ? "text-green-500" : "text-red-500"}`}>
              {commodity.change >= 0 ? "+" : ""}

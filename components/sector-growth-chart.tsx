...
export function SectorGrowthChart() {
  ...
  useEffect(() => {
    if (filteredData.length) {
      ...
      const sectorGrowthData = Object.entries(sectors).map(
        ([sector, stocks]) => {
          ...
          const sectorGrowth =
            stocks.reduce((sum, stock) => {
              ...
              const lastValue = Number.parseFloat(
                filteredData[filteredData.length - 1][stock]
              );
      ...
      sectorGrowthData.forEach((sector) => {
        config[sector.name] = {
          label: sector.name,
          color:
            sector.growth >= 0 ? "hsl(143, 85%, 40%)" : "hsl(346, 85%, 40%)",
        };
      });

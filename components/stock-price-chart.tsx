...
export function StockPriceChart({
  ...
  const [stockConfig, setStockConfig] = useState<Record<string, { label: string; color: string }>>({})
  ...
  useEffect(() => {
    if (!techOnly && !cryptoOnly && !indicesOnly && !commoditiesOnly && !financeOnly) {
      setSelectedStocks(["S&P_500_Price", "Nasdaq_100_Price"])
    }
  }, [techOnly, cryptoOnly, indicesOnly, commoditiesOnly, financeOnly, setSelectedStocks])
  ...
  useEffect(() => {
    ...
    else if (commoditiesOnly) displayStocks = commoditiesStocks.map((s) => s.field)
    ...
    const formattedData = filteredData
      ...
      .map((item) => {
        const dataPoint: any = { date: new Date(item.Date).toLocaleDateString() }
  ...
  }, [filteredData, selectedStocks, techOnly, cryptoOnly, indicesOnly, commoditiesOnly, financeOnly, setSelectedStocks])
  ...
  return (
    <Card>
      <CardHeader className="flex flex-row items-start justify-between">
        ...
        {!techOnly && !cryptoOnly && !indicesOnly && !commoditiesOnly && !financeOnly && (
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56">
              ...
              <DropdownMenuGroup>
                <DropdownMenuLabel className="text-xs text-muted-foreground">Indices</DropdownMenuLabel>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuLabel className="text-xs text-muted-foreground">Tech Stocks</DropdownMenuLabel>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuLabel className="text-xs text-muted-foreground">Cryptocurrency</DropdownMenuLabel>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuLabel className="text-xs text-muted-foreground">Commodities</DropdownMenuLabel>
              ...
              <DropdownMenuSeparator />
              <DropdownMenuGroup>
                <DropdownMenuLabel className="text-xs text-muted-foreground">Banking & Finance</DropdownMenuLabel>
